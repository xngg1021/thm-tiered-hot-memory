...
Bounded forensic rows include dense scores/deltas/ranks, selected state, budget and cutoff information when the source receipt contains them. Legacy sparse/fused numeric components are unavailable and remain null; these receipts cannot establish near ties as the sole cause. Current evidence is consistent with near-boundary floating-point sensitivity. No source text is copied into forensic output.
