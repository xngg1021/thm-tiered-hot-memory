...
Performance decomposition reports FTS, query embedding, matrix loading, scoring, fusion, expansion, packing and materialization with overlap semantics. Transfer is nested within scoring; FTS within sparse. Shared batch clocks are reported separately: chunk size determines each batch, including distinct batches with identical clock values; matrix loading is counted once per native scope/mode/budget call. Incomplete chunks remain unresolved rather than producing fabricated totals. Unmeasured orchestration residual remains unknown. The measured GPU document embedding gain does not imply the same end-to-end gain.
