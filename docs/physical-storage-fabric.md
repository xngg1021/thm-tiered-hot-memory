...
External placement is optional and explicit. Re-embedding clears its pointer while retaining immutable files. A stale or corrupt placement fails closed unless the existing scheduler was explicitly configured for sparse fallback. No cross-profile dense fallback is introduced.
...
Reference sidecars bind exact dataset bytes/subset, local model manifest, backend/device/precision/package settings, reference policy, semantic implementation closure, counter identity and protocol revision. `--reference-dir /previous/run` only reuses matching reference artifacts; stale keys/checksums fail. Receipts carry `reused=true`, source artifact SHA, complete key and provenance. The semantic dependency closure is explicit in `research/runtime/bounds.py`; changes affecting reference semantics must extend it. Documentation/scheduler/probe-only changes do not invalidate that semantic key. Pilots and measured winners remain separate new evidence.
