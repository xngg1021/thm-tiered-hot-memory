...
def validate_telemetry_event(raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if event["kind"] == "prefetch":
        ...
        forbidden = {
            "used", "mode", "confidence", "avoided_miss", "avoided_extra_tokens",
            "avoided_extra_tool_calls", "avoided_extra_latency_ms", "avoided_extra_cost_usd",
        }
...
def aggregate_telemetry(events: Iterable[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    avoidable_miss_events = [e for e in normalized if e["kind"] in MISS_KINDS and e["avoidable"]]
    ...
    for item_id in sorted(per_item):
        ...
        row["need_task_rate"] = _ratio(len(row["demand_task_ids"]), len(task_ids))
        ...
        row["prefetch_coverage_observed"] = _ratio(
            row["prefetch_avoided_misses"], avoidable + row["prefetch_avoided_misses"]
        )
    ...
    return {
        ...
        "avoidable_miss_penalty": {
            ...
            "extra_tool_calls": sum(e["extra_tool_calls"] for e in avoidable_miss_events),
            "extra_latency_ms": sum(e["extra_latency_ms"] for e in avoidable_miss_events),
