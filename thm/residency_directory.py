...
def _locator_topic(locator: str) -> str:
    """Derive a display label from the locator itself, never entry key/summary text."""
    path = PurePosixPath(locator)
    stem = path.stem or path.name
    return _sanitize_field(stem.replace("_", " ").replace("-", " "), 80) or "resource"
...
def project_warm_directory(entries: Sequence[Mapping[str, Any]], *, now: date,
    ...
    """Derive a compact T1 locator directory; never emit keys/summaries as evidence."""
    ...
    for raw in entries:
        ...
        if locator is None:
            unlocatable.append(item_id)
            continue
        topic = _locator_topic(locator)
        scope = _entry_scope(raw)
        latest = _latest_entry_day(raw)
        key = (scope, locator)
        ...
        if latest >= row["latest"]:
            ...
            row["revision"] = (
                source_hash[:12] if isinstance(source_hash, str) and source_hash else "-"
            )
        ...
        row["cost_priority"] = max(
            row["cost_priority"], {"low": 1, "med": 2, "high": 3}[cost_class]
        )
    rows: list[dict[str, Any]] = []
    for row in groups.values():
        line = (
            f"{row['topic']} | {row['scope']} | {len(row['item_ids'])} current | "
            f"{row['latest'].isoformat()} | rev:{row['revision']} | {row['locator']}"
        )
        units = count_units(line + "\n")
        if type(units) is not int or units < 0:
            raise ResidencyError("count_units: expected nonnegative integer")
        rows.append(
            {
                "topic": row["topic"],
                "scope": row["scope"],
                ...
                "current_items": len(row["item_ids"]),
            ...
            dropped.append(
                {
                    ...
                    "locator": row["locator"],
                    "units": row["units"],
                    "reason": "directory_budget",
                }
            )
    return {
        "status": "OK",
        "tier": "T1",
        "projection": "locator_only",
        "topic_source": "safe_locator_stem",
        "lines": [r["line"] for r in selected],
        "selected": selected,
        "dropped": dropped,
        "unlocatable_item_ids": sorted(unlocatable),
        "excluded": sorted(excluded, key=lambda r: r["item_id"]),
        "budget": budget,
        "budget_used": used,
