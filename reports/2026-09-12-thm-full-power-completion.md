...
A Linux 3.14 CI trace identified permission loss while sampling the exiting worker leader’s procfs I/O. The bounded cleanup path now confirms actual leader termination within 0.2 seconds before discarding that disappearing sample; live-leader and helper permission failures remain explicit errors, and acceptance still requires the independent worker lifetime receipt. Regression covers all three outcomes.

Before any leader exit wait, Linux accounting now captures process-group membership. If a helper is present and any member’s read is denied, the result is rejected before waiting; a disappearing helper cannot turn that group into an accepted empty sample. The regression supplies an exit wait that would remove the helper and proves it is never invoked.
